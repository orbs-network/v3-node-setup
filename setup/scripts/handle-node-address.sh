...
if [[ ! -f $keys_path || $* == *--new-keys* ]]; then
  ...
  chmod +x $ORBS_ROOT/setup/generate_wallet.py
  while true; do
      ...
      if [[ -z "$input" ]]; then
          ...
          $ORBS_ROOT/setup/generate_wallet.py --path $keys_path --new_key
          break
      elif [[ $input =~ ^(0x)?[0-9a-fA-F]{64}$ ]]; then
          echo -e "${YELLOW}\nThe private key is valid. Importing the wallet...${NC}"
          $ORBS_ROOT/setup/generate_wallet.py --path $keys_path --import_key $input
          break
      else
