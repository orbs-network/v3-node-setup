...
chmod +x $ORBS_ROOT/setup/generate_env_files.py
env_dir=$ORBS_ROOT/deployment
shared_name=shared.env
env_file=.env

if [[ ! -f "$env_dir/$env_file" || $* == *--new-keys* ]]; then
  $ORBS_ROOT/setup/generate_env_files.py --keys $keys_path --env_dir $env_dir --env_file $env_file --shared $shared_name
